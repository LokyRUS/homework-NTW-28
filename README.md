...
3) Ситуация в таблице маршрутизации не оптимальна, Reference bandwidth нужно изменить на маршрутизаторах R1;R2;R3;R4, для перерасчета метрики
...
~~После изменения, OSPF предложить установить, в таблицу маршрутизации, межзонный маршрут  R1-R3-R4, как самый дешовый и оптимальный.
Остальные машруты (R1-R5, R1-R2-R4 )буду использоваться как резервные, так как они так же являются межзонными и выбор будет производиться на основании метрик маршрутов~~

4) После изменения, OSPF предложить установить внутренний маршрут зоны, в таблицу маршрутизации, внутрезонный маршрут R4-R5, не учитывая стоимость межзонных маршрутов.
Остальные машруты (R1-R3-R4, R1-R2-R4 )буду использоваться как резервные, так как они являются межзонными.

## Марштуры: 
- Приоритетный внутренний маршрут зоны (intra-area) - O R4-R5;
- Резервный межзональный (inter-area) - O IA R1-R3-R4;
