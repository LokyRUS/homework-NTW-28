...
Если существует установленная BGP- сессия, то гарантированно не передавались сообщения `Open`, так как отправляется при открытии сессии. Так как соединение стабильно , исходя из вопроса, `natification`  тоже не отправлялось.
